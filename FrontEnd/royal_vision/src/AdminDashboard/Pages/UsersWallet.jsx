...
const UsersWallet = () => {
  ...
  useEffect(() => {
    const fetchWallets = async () => {
      try {
        const response = await fetch('https://overland-23a4680d9e06.herokuapp.com/api/admin/userswallet');
        if (!response.ok) {
    ...
    <div className='space-y-4'>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {wallets.map((wallet) => (
          <div
            key={wallet.id}
            className="bg-white dark:bg-[#1E2140] rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden  dark:border-gray-700"
          >
            <div className="p-6">
